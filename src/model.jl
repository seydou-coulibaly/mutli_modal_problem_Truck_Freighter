...
function setmodel(solverSelected,Q,alpha,T,width,s,V,P,J,Js,Jl,Jls,nodes,latitude,longitude,q,a,cout)
    ...
    # 1) cas où i in J
    @constraint(ip,[i in union(J,P)], sum(Xb[i,j] for j in V) == sum(Xb[j,i] for j in V))
    ...
    @constraint(ip, sum(Xb[i,n] for i in union(J,P)) == 1)
    ...
    @constraint(ip,[i in J, j in V], ws[i] + s + alpha*cout[nodes[i],nodes[j]] <= ws[j] + (1-Xs[i,j])*T)
    @constraint(ip,[i in setdiff(V,J), j in V], ws[i] + 0 + alpha*cout[nodes[i],nodes[j]] <= ws[j] + (1-Xs[i,j])*T)

    @constraint(ip,[i in J, j in V], wbi[i] + s + cout[nodes[i],nodes[j]] <= wbi[j] + (1-Xb[i,j])*T)
    @constraint(ip,[i in setdiff(V,J), j in V], wbi[i] + 0 + cout[nodes[i],nodes[j]] <= wbi[j] + (1-Xb[i,j])*T)

    # Ajout de la fenetre de temps sur wbo
    @constraint(ip,[i=1:m, j in V], wbo[i] <= ws[j] + (1-Xs[P[i],j])*T)
    # @constraint(ip,[i=1:m, j in V], wbi[i] + alpha*cout[nodes[i],nodes[j]]  <= ws[j] + (1-Xs[P[i],j])*T)

    @constraint(ip,[i=1:m, j in V], wbo[i] + cout[nodes[P[i]],nodes[j]] <= wbi[j] + (1-Xb[P[i],j])*T)
    # constraint 4 :
    ...
    @constraint(ip,[i in P, j in V], Xs[i,j] <= sum(Xb[i,k] for k in V))
    @constraint(ip,[j in P, i in V], Xs[i,j] <= sum(Xb[k,j] for k in V))
